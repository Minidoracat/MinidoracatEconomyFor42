[h1]💰 Minidoracat Economy for B42[/h1]
[h3]By Minidoracat[/h3]

[hr][/hr]

[h2]✨ このMODについて[/h2]
2種類の通貨、取引センター、オークション、Discordポイントからゲーム内通貨への交換を提供する予定の経済システムです。

[h2]🚧 開発状況[/h2]
現在はMODプロジェクトの基本構成のみで、ゲーム内の経済機能はまだ実装されていません。

[h2]🧰 実装予定の機能[/h2]
[list]
[*] [b]2種類の通貨[/b]：ゲーム内通貨とプレイヤー残高を管理
[*] [b]取引センターとオークション[/b]：プレイヤーによる出品、購入、取引に対応
[*] [b]Discordポイント交換[/b]：Discordポイントをゲーム内通貨へ交換
[/list]

[h2]📋 MOD情報[/h2]
[list]
[*] [b]Workshop ID:[/b] 未割り当て
[*] [b]Mod ID:[/b] MinidoracatEconomyFor42
[*] [b]対応バージョン:[/b] Build 42.20.4+
[*] [b]シングルプレイ / マルチプレイ:[/b] 両方に対応
[/list]

[h2]💬 フィードバックとコミュニティ[/h2]
[list]
[*] [url=https://discord.gg/Gur2V67]Discordコミュニティ[/url]
[/list]


[h2]☕ 作者を応援[/h2]
この MOD は今後もずっと無料です。気に入ったらコーヒーを一杯おごってください。支援はサーバーと MOD 開発に使います。
[url=https://ko-fi.com/minidoracat][img]https://raw.githubusercontent.com/Minidoracat/workshop-resources/refs/heads/main/badges/badge_kofi.png[/img][/url]

[b]#Minidoracat[/b]
