...
| 生存里程碑 | 依本季單一角色已記錄的生存天數自動發獎；帳號每季每門檻一次，死亡不重發，換季重新計時、不帶前季天數。 |
| 生存賽季 | 管理員手動換季或依現實天數自動結束；長度設定立即從本季原開始時間重算截止並同步在線玩家，0＝手動，不重開本季或重置成績；會立即過期的縮短須改用明確換季。停服照計時，逾期重啟只開一個新季。歷季只保存每帳號最長單命與季別期間，不清餘額、不猜舊成績。 |
| 生存排行榜 | 與即時資產榜共用入口，選生存類別與當季／歷季，以該季最長單命的已記錄遊戲分鐘排名；死亡保留最佳，不加總多條命。 |
...
| 電台 | 交易站定時廣播行情文字摘要；管理員可另行開啟預設關閉的原生語音／一般聊天中繼，與摘要共用頻率及範圍。玩家仍依本體 PTT／VAD 及收訊規則，ATM 不提供。操作見 README「交易站電台」，引擎契約與驗收界線見主規格 §17.3。 |
...
| 7 | ~~每日獎勵全服上限分配~~ | **已定：簽到不設全服上限**（設定保留、預設 0＝不限） | 每帳號每日一次×固定金額，總量已被在線人數綁死；里程碑每帳號每季一次；全服每日上限只留給 Discord 存入與整合 MOD 來源。「日」＝現實日（`getTimestampMs()` 壁鐘；換日時刻＝沙盒 `RewardDayResetHour` 當地小時＋`RewardTimezoneUTC` 時區（UTC 偏移小時），預設台灣（8）00:00＝UTC 16:00，不依賴主機時區；日鍵是當地日期，2026-09-06 定案） |
...
| 11 | ~~管理員調整餘額的門檻~~ | **已定：沙盒預設＋原生角色管理者可在面板覆寫**（2026-09-12 修訂） | 單筆 5,000、每位管理員每日加／減各 10,000、非空原因（最多 1,000 字）、全服每日加／減各 50,000；修改限制須 `RolesWrite`，一般經濟寫入權不能自行提高上限 |
| 12 | ~~誰能看玩家餘額與收據~~ | **已定：精確角色名單** | 預設 `moderator` 唯讀、`admin` 可寫，不按位階繼承；原生角色管理者另有唯讀入口以修正授權，無自動經濟寫入權 |
...
| 調整餘額 | 選幣別、±金額、必填非空原因（最多 1,000 字）、前後餘額確認（帶錢包版本）；單筆與每日加／減各自上限，自身帳戶預設拒絕、須另授權；不可負餘額、不可動保留款；每筆是一條有理由的帳本 posting，無法刪改，只能反向再調 | 寫 ModData＋事件檔；Watchcord 輪詢到即推 Discord 管理頻道告警；遊戲內調整與 Discord 退點由不同人負責 |
...
權限：`AdminRoles`／`ReadOnlyRoles` 精確比對伺服器角色名稱，預設 `admin` 可寫、`moderator` 唯讀。原生角色管理權（`RolesWrite`）才能修改調帳上限及授權清單；三份名單直接從本服既有角色選取，不必手輸入。自身調帳另需同時列入 `AdminSelfAdjustRoles`，預設無人獲授權；不能凍結自己。**每個操作都在伺服器重驗**，設定沿用世界存檔耐久性。完整歷史、匯出與跨期報表不在遊戲內做——Watchcord 有完整帳本。
...
**A14b 更新（2026-09-13）**：原生交易站 HAM 整合已實作；上述階段安排保留作歷史，最新契約見主規格 §17.3。已驗證原生裝置資料及離線介面，尚未完成雙客戶端實際音訊／聊天送達驗收。
