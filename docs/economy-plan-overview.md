...
| 5 | 信箱是否跨角色死亡保留、待領是否永不過期 | 保留＋永不過期／保留＋90 天／死亡清空 | 保留＋永不過期（明示為刻意玩法） |
| 6 | 拍賣遇維護或長停的政策 | 延長／取消退款／照常結算 | 預告維護：延長；意外長停超過拍賣時長一半：取消退款 |
| 7 | 每日獎勵全服上限分配 | 先到先得（明示限量）／依時段預留 | 先到先得，UI 顯示剩餘 |
| 8 | 是否排 v2 提領 | 不排／排在 H 之後 | 不排；等 v1 上線觀察需求 |
| 9 | ~~ATM 是否為錢包寫入操作的必要條件~~ | 已併入 #4：所有寫入類操作都要在任一終端（ATM 或交易站）旁 | — |
| 10 | 倖存幣的初始數字尺度 | 簽到 30、里程碑 100–1,000、常見商品 10–5,000 | 沿用視覺稿的尺度 |
| 11 | 管理員調整餘額的門檻 | 單筆上限／每日淨上限／原因最短字數 | 5,000／10,000／10 字（都可在設定改） |
| 12 | moderator 能否看玩家餘額與收據 | 能（唯讀）／只有 admin | **已定（A13 實測）**：能（唯讀），操作限 admin；server 以角色名清單重驗，不用 capability（moderator 也有 `AddItem`） |
| 13 | 圖示同步管線放哪 | 抽到 UIFor42 `ImageSync`／Economy 自帶一份／只允許圖包 MOD 覆寫 | 抽到 UIFor42（兩個 consumer），就緒前 Economy 只出預設圖 |
...
3. 與 UIFor42 協調 `ImageSync` 抽出（NoticeBoard 的 `NBImage`／`NBImageCache`／server 掃描器）與管理分頁所需的單色圖示 key；就緒後補 A17。
