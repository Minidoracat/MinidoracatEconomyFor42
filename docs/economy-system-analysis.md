...
- **catalog 以檔案為唯一真相、面板直接寫檔**（同日第二輪修訂：原「檔案預設＋ModData 覆寫」作廢——檔案不隨世界存檔回滾，管理員的定價不該因崩潰消失）：`{cachedir}/Lua/MinidoracatEconomy/catalog.json`（整份 JSON：`{"items":[{"id","item","qty","price","dailyCap","category","enabled"}]}`；缺檔時 server 寫出預設樣本；解析或驗證失敗沿用上一份並在面板顯示錯誤）；`id` `[A-Za-z0-9_-]{1,32}` 唯一、`item` 須 `ScriptManager.instance:FindItem` 可解析（`ScriptManager.java:1413-1419`）、`qty` 1–50、`price` ≥ 1（整數，`marketUnit` 幣）、`dailyCap` 0（不限）–1,000,000、最多 200 筆。面板 `admin.catalog{action=list|set|reload}`（寫閘門、稽核）：`set` 改寫該列並重寫整個檔案（寫前比對磁碟 hash，手動改過未重載→`catalog_stale`；寫後重讀驗證），新增／刪除 SKU 仍靠手動改檔＋`reload`；任何改動 `Shop.pushAll()` 推送所有線上玩家的 `shop.list`。`revision`＝檔案 hash；`shop.buy` 帶 `revision`，不合即 `catalog_changed`（不靜默換價）。
