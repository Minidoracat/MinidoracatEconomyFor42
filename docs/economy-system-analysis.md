...
**進度（2026-09-07，Windows dedicated 42.20.4，離線 harness 281 條）**：B0 骨架 ✓、B1 帳本 ✓、B2 匯出 ✓、B3 companion ✓（TypeScript）、B4 貨幣 config ✓、B5 獎勵 ✓（實機簽到）、B6 錢包／獎勵頁 ✓（實機）、B7 管理面板 ✓（實機：唯讀角色、貨幣設定、稽核、系統、調帳、凍結／解凍）、B8 內建圖示＋自訂圖示同步 ✓（實機：下載、套用、移除回內建；A17 完成）、B9 整合 API ✓（harness；管理頁「整合」子分頁）、**B10 收尾 ✓**：ModData 回歸（2026-09-07 本機 1 帳號、5 筆收據、稽核環 8 筆）Economy 表 3,120 bytes、`Saving GlobalModData` 6 ms；每活躍帳號約 1.1 KB（wallets 186／receipts 環 5 筆 810／claims 119），idempotency 上限 2,000 筆約 80 KB——正式服月活 600 帳號估 0.7 MB、約 30 ms，遠低於 §20 的 8 MB 警戒；雙客戶端 E2E ✓（第二 client 以 `-nosteam` 另一帳號登入：簽到、管理員調帳、凍結／解凍皆過）；崩潰重複發幣測試 ✓（未存檔即殺 `java.exe`：餘額與稽核環回到上一份存檔、簽到可再領、companion 標 `rolledBack`）——並修正「從未存檔就崩潰的 epoch 在 ModData 無痕跡」：以 `epochs.json` 記錄每次啟動（含該次已標記的崩潰 epoch，未存檔連續重啟不重複寫 `epoch.rolledback`），客戶端收據檔列才標得出「已回滾」；回滾列在錢包「最近」、管理頁收據與稽核（含「已回滾」篩選）皆可見；凍結即時推送與紅色狀態帶；管理頁「設定」子分頁可直接改沙盒選項（§19 runtime 覆寫：`EC.OPTIONS` schema、`config.options`、`admin.option`；管理額度與角色名單鎖定為伺服器檔專屬；獎勵類改動即時推 `rewards.state`）；視窗預設 3/4 螢幕、標題列「重設大小」。**未做、留待後續階段**：規則五（死亡 carryOver）→ 階段 D；companion 寫 `durable.json` 供客戶端逐列顯示「已存檔」（選項 2）→ 有需求再做。
...
- **catalog 以檔案為唯一真相、面板直接寫檔**：`{cachedir}/Lua/MinidoracatEconomy/catalog.json` 為 `{"items":[...]}`；每列含 `id/item/qty/category/enabled/dailyCap/dailyCapScope/buybackCap` 及 `prices[currency]={price,bidPrice,enabled,buyback}`。舊單幣扁平價格僅在讀檔邊界轉換，既有檔讀取失敗不寫預設。`id` 為 `[A-Za-z0-9_-]{1,32}`、最多200筆；`item` 須由 server `ScriptManager.instance:FindItem` 解析（`ScriptManager.java:1413-1419`），每份1–50件。`admin.catalog{action=list|set|add|batch|reload}` 沿用讀／寫權限與稽核；單筆與批次共用整份驗證、expected revision、磁碟hash、一次寫入／讀回，再 `Shop.pushAll()` 推送快照。既有商品 `id/item` 不可由面板換成別項；`qty/category` 可改。`revision` 為檔案hash，購買不匹配回 `catalog_changed`，不靜默換價。
- **購買（現行）**：`shop.buy{id, count(1–10), currency, revision, requestId, acceptMail?}` 先驗基本形狀與既有請求身分，成功重送沿用帳本的有限冪等窗口；新操作核對商品報價、revision、終端距離、凍結、單次物品數上限100、限購份數、信箱空位及餘額。`M.prepare` 建物並量重；放不下且未同意寄存即拒絕、零扣款。成功 `L.debit(...,{kind="shop_buy"})` 後記錄份數並建立信件，放得下立即交付，否則依同意寄存；已付款的信件占用額度，之後領取或部分交付不再加扣份數。事件 `shop.purchase`、收據 kind `shop_buy`。
- **三種互斥限購（2026-09-15）**：保留已發布設定欄名 `dailyCap`（0不限、1–1,000,000份）與 `dailyCapScope`；模式為 `player`（每日每人、缺省）、`global`（每日全服）、`lifetime`（每人永久），其餘值拒絕。`Shop.used(username,id,ms)` 是快照、購買守門及回覆的共用讀點；`used` 為目前模式已用份數，有限購才有 `remaining=max(0,cap-used)`。全服 `shop.stock` 同時送 `used/remaining`，個人用量不作全服廣播。
- **永久累計來源**：`shopLifetime[username][skuId]` 與既有每日 `shopDaily` 同屬世界 ModData；每次新版成功購買同時計入兩者，包括當時未開限購的商品。永久累計不採用每日31天清理，因為切回永久模式仍需保留全部已計份數；不從不完整日桶或可能已回滾的事件檔回補舊資料。模式／價目／每份件數／角色／賽季變更不清計數，世界存檔回滾則連同帳款一起回滾。下修上限不刪已購數，也不追回物品；同 ID 保留、新 ID 另計。
- **購買事件附加欄位**：`shop.purchase` 保留原身份及金額欄，追加 `capScope`（當次模式）、`cap`（當次上限）、`usedAfter`（該模式成交後已用份數）、`lifetimeUsed`（該帳號商品的永久累計）。用量單位均為份數，成交前該模式用量為 `usedAfter-count`；舊行缺欄保持未知。沿用既有事件 writer，不新增計數 JSON、重播恢復器或 companion 日報規則。
...
- 最小信箱（delivery claim、背包滿保留 READY，任一終端可領）在此階段先上，供交付失敗與退件使用。**上限規則（2026-09-06 主持人定案）**：信箱是帳號層資料、只存快照（§19.7 規則一），每帳號容量由沙盒 `MailboxPerAccount` 決定（預設 50，管理頁可調）、全服 10,000。**2026-09-08 修訂**：一格＝一筆待領取**或**一筆掛在市場上的刊登（上架就佔格），購買／系統商店購買／上架都需要一個空位（提示「信箱已滿，先領取」）；取消、到期、管理員下架只是把刊登那一格換成待領取，**一律成功且永不超過上限**；待領項永不過期、跨角色死亡保留，領取即結清消失（信箱數只算未領）。
...
**白名單實作形狀（2026-09-07 修訂，取代上面「每筆白名單列」的草案格式）**：`whitelist.json` = `{"categories":[DisplayCategory…],"types":[fullType…],"excludeTypes":[…],"modDataKeys":[…]}`，`excludeTypes` > `types` > `categories`；檔案是唯一真相（同 `catalog.json`）——管理頁「白名單」分頁一次改一個分類（開關）或一件物品（允許／禁止／回到依分類），`admin.whitelist{action=set}` 直接寫回檔案、重讀、稽核，檔案被手改則回 `whitelist_stale` 要求先重載；分頁的物品宇宙由 client `getScriptManager():getAllItems()` 建立、可關鍵字搜尋（譯名／原文／ID／分類名）。固定拒絕改以 `script:isItemType(ItemType.X)` 判定（`EC.LISTING_FIXED_TYPES`：Container／Clothing／Key／KeyRing／Moveable／Map／AlarmClock／AlarmClockClothing／Animal；**無線電**的 DeviceData 隨快照走，不在固定拒絕內）。**2026-09-07 第二次修訂**：modData 不再是拒絕理由（原版自己就寫 `customName`／`condition:*`，`InventoryItem.java:3253-3271`，一般木板都被擋）——改為有界深拷貝隨快照（≤32 純量、深度 3、字串 ≤128，超過才拒 `moddata_too_big`），`modDataKeys` 欄位移除；自訂名稱 round-trip；會腐敗食物可上架，快照存上架時 `getWorldAgeHours()`，重建時把託管期間的世界時數依 `FoodRotSpeed` 加進 `age`（`Food.java:739-786` 是倒數制，否則託管＝免費冰箱），只擋已腐敗；讀過的書可上架（原版開始閱讀時重設頁數，`ISReadABook.lua:350-351`）。容器（含內容物遞迴）、鑰匙（keyId）、家具（worldSprite）、衣物（血／髒／濕／破洞／補丁／顏色）技術上都可重建，主持人本輪只選無線電，其餘維持固定拒絕——原本比對 `getCategory()` 主類別字串，但 `Moveable`／`Radio`／`MapItem` 沒有覆寫 `getCategory`（回 `"Item"`，`InventoryItem.java:681-683`），無線電在 `Electronics` 分類開啟時本可上架、重建會丟頻道與電池。
...
**進度（2026-09-08，語音整合更新於 2026-09-13）**：信箱容量採 `MailboxPerAccount`＋刊登佔格；codec 已涵蓋流體、修理、modData、食物老化與無線電 DeviceData。`ECRadio.lua` 的行情文字摘要與拍賣即將到期提示已完成；`RadioIntervalMinutes=0` 只關閉摘要。A14b 原生 HAM 伴隨裝置及獨立收音開關已加入，API／存讀與離線介面驗證不等於雙客戶端實際音訊驗收，契約與限制見 §17.3。
...
**進度（2026-09-08，離線 harness 565 條、companion 測試 7 個）**：**server ✓**（`ECExchange.lua`，鏈 `… ← ECAuction ← ECExchange ← ECAdmin`）：`OnTickEvenPaused` 每 2 s `listFilesInZomboidLuaDirectory("MinidoracatEconomy/inbox")`（每輪 ≤50 檔、只認 `^[A-Za-z0-9_-]+\.json$`）→ `getFileReader` 讀單行 JSON → 以 `md.exchange.tombstones[orderId]` 冪等（≥5,000 筆時新單 **fail closed**、檔案留著、不逐出）→ 驗證：幣別註冊且 `exchange` 存在、`enabled`、`(rateVersion, rateSnapshot)` 為目前版本或最近 8 版之一（`Cfg.rateAccepted`；`setExchange` 每次把舊 `{rateVersion, pointsPerCoin}` 推進 `exchange.ring`）、`amount == floor(points / rateSnapshot)`、`perOrderMin ≤ amount ≤ perOrderMax`、每人／全服每日 gross 上限（`md.exchange.daily[day]`，以獎勵日鍵）→ `L.credit(username, currency, amount, "EXTERNAL_DISCORD_" .. currency, {kind="exchange_deposit", requestId="discord:"..orderId})` → 事件 `exchange.deposited{orderId, username, currency, points, amount, rateSnapshot, rateVersion, txId, creditSeq}`；任一驗證不過 → tombstone `failed` ＋ `exchange.failed{orderId, reason=rate_mismatch|amount_mismatch|order_range|daily_cap{scope}|currency_disabled|not_exchangeable|unknown_currency|invalid_order|balance_cap|…}`（對該 orderId 終態）。線上玩家收 `exchange.notice`＋`wallet.changed`；離線者從對帳單看到 `Kind_exchange_deposit`（不另做登入補通知）。檔案消失的 tombstone 每 60 s 清掉（companion 只在事件 durable 後刪檔，故 tombstone 本身已落盤）。壞檔（非 JSON）只記 log、不 tombstone、留給主機處理。**啟動與每次 config 變更發 `exchange.config{currencies[]}`**（無 icon bytes、無 ring；先於 `admin.config`）供 companion 投影。**companion ✓**（`orders.ts`）：`GET /currencies`（最新未回滾的 `exchange.config`）、`POST /orders`（`orderId` `^[A-Za-z0-9_-]{1,64}$`；`username ∈ accounts.forSteam(steamId64)` 否則 403；幣別／啟用；`rateVersion`／`rateSnapshot` 必須等於目前投影否則 409 `rate_changed` 並附 `currencies`；`amount == floor(points / rate)`、範圍；`inbox/<id>.json.tmp` → rename；202 `{orderId, status:"pending"}`；同 id 同 payload 回原結果／終態、不同 payload 409 `order_conflict`；`orders.json` 索引保留 30 天，讓事件離開記憶體後仍能辨識重送）、`GET /orders/{id}`（`pending|deposited|failed|unknown` ＋ `durable`、`inbox`、`event`）、每輪 `sweep()` 刪除結果已 durable 的 inbox 檔（回滾的結果永不 durable → 檔案留著給 Lua 重放）、`/health.orders{inboxFiles, indexed}`。**未做**：Watchcord 端（`shop_items.type='pz_currency'`、`GET /ledger` poller、`admin_grant` 反向）在 Watchcord repo；社群幣的遊戲內 sink（沒有 sink 前不對玩家開放存入——現行以貨幣 `enabled=false` 把關）；companion 正式部署（PM2、`BIND` 內網 IP、`HMAC_SECRET`、ufw）。harness 情境三十四含回滾重放恰一次、tombstone 上限 fail-closed、版本環。
...
- **原生語音／聊天整合（2026-09-13）**：`RadioRelayEnabled` 預設 false；明確開啟後，server 在每座已登錄 `kind=trade` 的終端同格建立原生 `IsoRadio`，外觀為貓娘肩側／機台上緣的小喇叭，不替換既有 entity、不增加物品腳本。操作步驟見 README「交易站電台」。shared `ECTradeRadio` 以 `IsoRadio`＋`DeviceData.deviceName` 識別自有裝置；原生欄位隨 `IsoWaveSignal.save/load` 保存（`:308-330`；`DeviceData.java:1237,1278`），不以 modData、貼圖或一般物件名稱決定所有權。
- **建立與更新**：`cloneDeviceDataFromItem("Base.HamRadio1")` 取得裝置資料後先 `setDeviceData`，完成頻率、範圍、twoWay、電力及開機設定後才 `AddSpecialObject`／`transmitCompleteItemToClients`（`IsoWaveSignal.java:98-115,143-155,333-340`；`IsoGridSquare.java:6185-6225`；`IsoObject.java:4552-4560`）。持續電力不生成可取出的電池。更新走單物件移除後重建，不能重送 Add 封包冒充更新；自有單格裝置使用 `transmitRemoveItemFromSquare(obj,false)`，避免原生多格展開越過所有權邊界（`IsoGridSquare.java:6315-6362`）。沒有額外強制存檔。
- **喇叭外觀**：同一 pack／tiledef 末尾新增 `MinidoracatEconomy_speaker_catgirl_0..3` 與 `MinidoracatEconomy_speaker_terminal_0..3`；舊 sheet 1／2 與八張貼圖不變，新 sheet 3／4 不給搬移、拆解、碰撞、表面或 `CustomItem` 屬性。位置直接編入透明 128×256 cell，來源格與語音範圍不變；舊自有 HAM 經既有外觀不符分支單件移除後重建，沒有透明裝置或大機身 fallback。
- **共用市場預設**：client `ECRadioPresets.lua` 在原生 `RWMChannel` 的下拉清單追加固定項，不改 `DevicePresets`／容量／物品存檔，也不掃描背包或世界。選項用原生 `addOptionWithData(text,data,tooltip)` 身分識別（`ISComboBox.lua:435-498`），不只靠索引；原生新增預設回傳的 `_selected` 保留玩家項語意。調頻沿 `RWMPanel.doWalkTo`＋`ISRadioAction("SetChannel")`（`:43-52`／`:55-80,173-188`），直接使用整數 kHz，避開編輯滑桿的 0.2 MHz 取整（`ISSliderPanel.lua:181-192`）。設定變更只刷新選單，不自動調頻；舊展示與新頻率不一致時拒絕排程，已排程的原生動作保留當時值。共用項不允許單台 edit/delete，玩家自己的項目不變。適用原生電台介面與裝置頻段，第三方自製介面不假稱已整合。
